...
- **工具条（`SettingsBar.swift`）每个控件、每个 `Text` 都必须 `.fixedSize()`**，
  否则窗口变窄时会被挤压变形（中文标签会被压成竖排）。加新控件时照抄现有写法。
- **加/删工具条控件后必须重新核对 `ContentView` 的 `minWidth`**（当前 880）。
  动图按钮的标签是动态的（含抽帧比例和速度），按最长情况估。
- **窗口上下两条横条的高度统一走 `BarMetrics`**（工具条 / 状态栏 / 对比面板的头尾）。
  高度写死是为了让中缝两侧的分隔线对齐，控件撑高了要改常量，不要只改一边的 padding。
- **在 SwiftUI 菜单的动作里不要直接 `NSSavePanel.runModal()`**，菜单的事件跟踪没结束，
  关掉面板后那个菜单就再也点不开了。要推迟一次 runloop 再用 `beginSheetModal` 呈现。
