...
- 静态 WebP：ImageIO 解码 + libwebp 重编码（无损 / 有损双候选取更优）。
  WebP 没有可解析的质量元数据，所以**省下不足 10% 时保留原图**，避免已优化过的
  WebP 反复处理造成代际劣化
- **三个转换开关**，语义统一为「额外输出一份 X」，不替换主输出：
  - **WebP** — PNG / JPG 额外输出 `名字.webp`（同名换扩展）
  - **JPG** — PNG / 静态 WebP 额外输出 `名字-jpg.jpg`（透明部分平铺白底）
  - **PNG** — JPG / 静态 WebP 额外输出 `名字-png.png`（真无损，保留透明；
    不受全局质量档影响）
  - 目标文件名会覆盖列表中已有图片时（如同目录下已有 `名字.webp`）**自动跳过并提示**，
    不会误删你自己的文件
